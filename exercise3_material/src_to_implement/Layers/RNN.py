...
class RNN(BaseLayer):
    ...
    def calculate_regularization(self):
        if self.optimizer and self.optimizer.regularizer:
            self.regularzation_loss = self.optimizer.regularizer.norm(
                self.hidden_fcl.weights
            ) + self.optimizer.regularizer.norm(self.output_fcl.weights)
        
        return self.regularzation_loss
    ...
    def forward(self, input_tensor: np.ndarray) -> np.ndarray:
        ...
        previous_hidden_state = self.hidden_state if self.memorize else np.zeros(self.hidden_size)
        batch_size = input_tensor.shape[0]
        output_tensor = np.zeros((batch_size, self.output_size))
        
        for i in range(batch_size):
            combined_input = np.concatenate((input_tensor[i], previous_hidden_state)).reshape(1, -1)
    ...
    def backward(self, error_tensor: np.ndarray) -> np.ndarray:
        ...
        gradient_previous_hidden_state = np.zeros(self.hidden_size)
        ...
        if self.optimizer:
            ...
            self.output_fcl.weights = self.optimizer.calculate_update(
                self.output_fcl.weights, self.output_fcl.gradient_weights
            )
